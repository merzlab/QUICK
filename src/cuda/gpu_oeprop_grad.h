...
__device__ void add_oeprop_grad(unsigned int I, unsigned int J, unsigned int II, unsigned int JJ, \
...
__device__ void iclass_oeprop_grad(unsigned int I, unsigned int J, unsigned int II, unsigned int JJ , unsigned int iatom, unsigned int totalatom, \
...
//        for(int iatom=0; iatom<totalatom; ++iatom){
            ...
            */
            ...
            oeprop_grad_vertical(I, J, II, JJ, Px-Ax, Py-Ay, Pz-Az, Px-Bx, Py-By, Pz-Bz, Px-Cx, Py-Cy, Pz-Cz, 1/(2.0*Zeta), store, YVerticalTemp);
    ...
    add_oeprop_grad(I, J, II, JJ, iatom, store2, storeAA, storeBB);

}

__global__ void get_oeprop_grad_kernel(){
  ...
  for (QUICKULL i = offset; i < jshell * jshell * totalatom; i+= totalThreads) {
    ...
    unsigned int idx   = i - iatom * jshell * jshell;
      ...
      if(devSim.mpi_boeicompute[idx] > 0){
        ...
        iclass_oeprop_grad(iii, jjj, ii, jj, iatom, totalatom, devSim.YVerticalTemp+offset, devSim.store+offset, devSim.store2+offset,\
