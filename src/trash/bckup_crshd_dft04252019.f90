...
!        quick_qm_struct%o(i,j)=0.d0
   ...
   if(.not.master) then
    do i=1,nbasis
     do j=1,nbasis
       write(*,*) "Madu: MPI before 1e O",i,j,quick_qm_struct%o(i,j)
     enddo
    enddo
   endif
...
if(master) then
  ...
  jmdt=3
   do Ibas=imdt,jmdt
  !write(*,*)
  !"ibas,mpirank,mpi_nbasisn(mpirank),mpi_nbasis",ibas,mpirank,mpi_nbasisn(mpirank),mpi_nbasis(mpirank,Ibas)
          !impi=mpi_nbasis(mpirank,Ibas)
      do Jbas= Ibas,nbasis
  ...
  jmdt=7
   do Ibas=imdt,jmdt
 !write(*,*) "ibas,mpirank,mpi_nbasisn(mpirank),mpi_nbasis",ibas,mpirank,mpi_nbasisn(mpirank),mpi_nbasis(mpirank,Ibas)
         !impi=mpi_nbasis(mpirank,Ibas)
      do Jbas= Ibas,nbasis
         quick_qm_struct%o(Jbas,Ibas) = 0.d0
         do Icon=1,ncontract(ibas)
...
!   enddo
 ...
 call MPI_BARRIER(MPI_COMM_WORLD,mpierror)
    ...
    write(*,*) "Madu: Slave sends this.."
    do i=1,nbasis
     do j=1,nbasis
       write(*,*) "Madu: MPI 1e O",i,j,quick_qm_struct%o(i,j)
     enddo
    enddo
