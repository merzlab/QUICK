...
!!!!!!!!!!!!!!!!!!!!!!!!!!Madu!!!!!!!!!!!!!!!!!!!!!!!!
!call MPI_BCAST(quick_qm_struct%gradient,natom*3,mpi_double_precision,0,MPI_COMM_WORLD,mpierror)
...
!------------Remove this when shared among all cores-----------------------
!call MPI_BCAST(quick_qm_struct%gradient,natom*3,mpi_double_precision,0,MPI_COMM_WORLD,mpierror)
!-----------Remove above when shared among all cores------------------------
call MPI_BARRIER(MPI_COMM_WORLD,mpierror)
...
!call MPI_BCAST(attraxiaoopt,size(attraxiaoopt),mpi_double_precision,0,MPI_COMM_WORLD,mpierror)
call MPI_BARRIER(MPI_COMM_WORLD,mpierror)

!if(.not.master) then
!write(*,*) "shape attraxiaoopt",shape(attraxiaoopt)
!stop
!endif
...
call MPI_BCAST(quick_qm_struct%gradient,3*natom,mpi_double_precision,0,MPI_COMM_WORLD,mpierror)
...
call MPI_BARRIER(MPI_COMM_WORLD,mpierror)
!!!!!!!!!!!!!!!!!!!!!!!!!!Madu!!!!!!!!!!!!!!!!!!!!!!!!
        write (*,*) "Madu STEP7 : ANALYTICAL GRADIENT:",mpirank
        do Iatm=1,natom
            do Imomentum=1,3
                write (*,'(I5,A1,7x,F20.10)')Iatm,cartsym(imomentum), &
                quick_qm_struct%gradient((Iatm-1)*3+Imomentum)
