...
if(MASTER) then  !Madu
! call copyDMat(oneElecO,quick_qm_struct%o,nbasis) !Madu
...
write(*,*) "E0=",quick_qm_struct%Eel
   ...
   if(quick_method%B3LYP)then
     ...
     !   IIimps=mpi_jshell(mpirank,imps)
     !   write (ioutfile,'(A22,2x,I5,2x,I5)') "Madu: mpirank, IIimps",mpirank,IIimps  
    do II=1,jshell
         do JJ=II,jshell
            Testtmp=Ycutoff(II,JJ)
            do KK=II,jshell
