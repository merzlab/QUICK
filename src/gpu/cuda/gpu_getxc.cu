    ...
    for (int jatm = 1; jatm <= devSim_dft.natom; jatm ++)
    {
        if (jatm != atm && wofparent != 0.0e0){
            ...
            if (confocal >= 0.64) {
                ...
                QUICKDouble gofconfocal = (35.0 * frctn - 35.0 * CUBE(frctn) + 21.0 * pow(frctn, 5.0) - 5.0 * pow(frctn, 7.0)) / 16.0;
...
__device__ QUICKDouble get_uw_ssd(const QUICKDouble gridx, const QUICKDouble gridy, const QUICKDouble gridz,
...
__device__ void sswanader(const QUICKDouble gridx, const QUICKDouble gridy,
        const QUICKDouble gridz, const QUICKDouble Exc, const QUICKDouble
        quadwt, QUICKDouble* const smemGrad, QUICKDouble* const uw_ssd, const
        int iparent, const int natom)
{
...
__device__ void sswder(QUICKDouble gridx, QUICKDouble gridy, QUICKDouble gridz,
        QUICKDouble Exc, QUICKDouble quadwt, QUICKDouble* smemGrad, int
        iparent, int gid)
{
    /*
...
//    printf("gridx: %f  gridy: %f  gridz: %f, exc: %.10e, quadwt: %.10e, iparent: %i \n",gridx, gridy, gridz, Exc, quadwt, iparent);
    ...
    for (int jatm = 0; jatm < devSim_dft.natom; jatm++) {
        ...
        if (jatm != iparent - 1) {
            ...
            QUICKDouble t = (-35.0 * CUBE(a + u)) / ((a - u) * (16.0 * CUBE(a) + 29.0 * SQR(a) * u + 20.0 * a * SQR(u) + 5.0 * CUBE(u)));
...
__device__ void becke(QUICKDouble density, QUICKDouble gx, QUICKDouble gy, QUICKDouble gz,
        ...
        QUICKDouble* dfdr, QUICKDouble* dfdgg, QUICKDouble* dfdggo)
{
    ...
    *dfdgg = 0.50 * gprimeofx / (x * rho4thirds);
}


__device__ void lyp(QUICKDouble pa, QUICKDouble pb, QUICKDouble gax, QUICKDouble gay, QUICKDouble gaz,
        QUICKDouble gbx, QUICKDouble gby, QUICKDouble gbz,
        QUICKDouble* dfdr, QUICKDouble* dfdgg, QUICKDouble* dfdggo)
{
