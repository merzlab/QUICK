...
contains
  ...
  subroutine iread(datasetname, ind, idata)
    ...
    INTEGER                 :: idata(1,1), ind, rank

    INTEGER :: hdferr

    INTEGER(HID_T)   :: file, dset, space_id, file_space ! Handles
    INTEGER(HSIZE_T) :: dims(1,1), npoints, dimn(2)
    INTEGER(HSIZE_T) :: length(2)
    !
    write(6,*)'ind =', ind
    dims(1,1) = ind
    rank=1
    ...
    !
    write(6,*)'idata at 88 =', idata(1,1)
!    call h5dread_f(dset, H5T_NATIVE_INTEGER, idata, dims, hdferr, file_space_id=file_space, mem_space_id = space_id)
    call h5dread_f(dset, H5T_NATIVE_INTEGER, idata, dimn, hdferr, space_id, file_space)
    write(6,*)'idata at 90 =', idata(1,1)
    if (hdferr /= 0) then
