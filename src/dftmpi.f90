...
!Calculate the electron kinetic energy operator 
if(master) then
    do Ibas=1,nbasis
      call get1eO(Ibas)
   enddo
...
!Set the elements of slave operators to zero. 
   if (.not.master) then
      do i=1,nbasis
         do j=1,nbasis
...
!Calculate the nuclear electron attraction
...
!Send the energy values and operator values from slave to master
 if(.not. master) then
   E1slave=quick_qm_struct%Eel
   call MPI_SEND(E1slave,1,mpi_double_precision,0,mpirank,MPI_COMM_WORLD,IERROR)
   ...
   do i=1,mpisize-1
      call MPI_RECV(E1slave,1,mpi_double_precision,i,i,MPI_COMM_WORLD,MPI_STATUS,IERROR)
      quick_qm_struct%Eel=quick_qm_struct%Eel+E1slave
...
!-----------------Madu----------------
!   if(master) then
!   write (*,*) "Madu: MPI version after 1e "
!   do i=1,nbasis
!      do j=1,nbasis
...
!   write(*,*) "E2=",quick_qm_struct%Eel
   ...
   !Set the operator value of slaves to zero
   if (.not.master) then
      do i=1,nbasis
         do j=1,nbasis
            quick_qm_struct%o(i,j)=0
