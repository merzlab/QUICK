...
extern "C" void gpu_upload_cutoff_matrix_(QUICKDouble* YCutoff,QUICKDouble* cutPrim)
{
   ...
   hipEventElapsedTime(&t_time, t_start, t_end);   
...
void prune_grid_sswgrad(){
        ...
        hipEventCreate(&t_endp);
        hipEventRecord(t_startp, 0);
#endif
	...
	gpu -> gpu_sim.uw_ssd = gpu -> gpu_xcq -> uw_ssd -> _devData;
        ...
        hipEventDestroy(t_startp);
        hipEventDestroy(t_endp);
#endif
...
void gpu_get_octree_info(QUICKDouble *gridx, QUICKDouble *gridy, QUICKDouble *gridz, QUICKDouble *sigrad2, unsigned char *gpweight, unsigned int *cfweight, unsigned int *pfweight, int *bin_locator, int count, double DMCutoff, int nbins){
	...
	hipMalloc((void**)&d_gpweight, gpu -> gpu_xcq -> npoints * sizeof(unsigned char));
	hipMalloc((void**)&d_cfweight, nbins * gpu -> nbasis * sizeof(unsigned int));	
	hipMalloc((void**)&d_pfweight, nbins * gpu -> nbasis * gpu -> gpu_basis-> maxcontract * sizeof(unsigned int));

	hipMemcpy(d_gpweight, gpweight, gpu -> gpu_xcq -> npoints * sizeof(unsigned char), hipMemcpyHostToDevice);
	hipMemcpy(d_cfweight, cfweight, nbins * gpu -> nbasis * sizeof(unsigned int), hipMemcpyHostToDevice);
	hipMemcpy(d_pfweight, pfweight, nbins * gpu -> nbasis * gpu -> gpu_basis-> maxcontract * sizeof(unsigned int), hipMemcpyHostToDevice);
...
*/
	...
	hipMemcpy(gpweight, d_gpweight, gpu -> gpu_xcq -> npoints * sizeof(unsigned char), hipMemcpyDeviceToHost);
	hipMemcpy(cfweight, d_cfweight, nbins * gpu -> nbasis * sizeof(unsigned int), hipMemcpyDeviceToHost);
	hipMemcpy(pfweight, d_pfweight, nbins * gpu -> nbasis * gpu -> gpu_basis -> maxcontract * sizeof(unsigned int), hipMemcpyDeviceToHost);
...
/*	int nblocks = (int) ((*gridb_count/SM_2X_XC_THREADS_PER_BLOCK)+1);
	...
	hipLaunchKernelGGL(test_xc_upload, nblocks, SM_2X_XC_THREADS_PER_BLOCK, 0, 0);
    ...
    if (incoreInt) {
        ...
        for (int i = 0; i<totalBuffer; i++) {
            ...
            if (currentInt >= availableERI) {
                ...
                //hipMemcpy(gpu->aoint_buffer[0]->_devData, gpu->aoint_buffer[0]->_hostData, currentInt*sizeof(ERI_entry), hipMemcpyHostToDevice);
                hipMemcpy(gpu->aoint_buffer[0]->_devData, intERIEntry + startingInt, currentInt*sizeof(ERI_entry), hipMemcpyHostToDevice);
        ...
        for ( int i = 0; i < bufferPackNum; i++) {
            ...
            for ( int j = 0; j < thisBuffer; j++) {
                ...
                if (bufferIndex >= availableERI) {
                    ...
                    //hipMemcpyAsync(gpu->aoint_buffer[0]->_hostData, gpu->aoint_buffer[0]->_devData, bufferIndex*sizeof(ERI_entry), hipMemcpyHostToDevice, stream[0]);
                    gpu->aoint_buffer[0]->Upload();
...
extern "C" void gpu_aoint_(QUICKDouble* leastIntegralCutoff, QUICKDouble* maxIntegralCutoff, int* intNum, char* intFileName)
{
    ...
    for (int iBatch = 0; iBatch < iBatchCount; iBatch = iBatch + streamNum) {
        ...
        for (int i = 0; i< streamNum && iBatch + i < iBatchCount; i++) {
            hipMemcpyAsync(&gpu->intCount->_hostData[i], &gpu->intCount->_devData[i],  sizeof(QUICKULL), hipMemcpyDeviceToHost, stream[i]);
            hipMemcpyAsync(gpu->aoint_buffer[i]->_hostData, gpu->aoint_buffer[i]->_devData, gpu->intCount->_hostData[i]*sizeof(ERI_entry), hipMemcpyDeviceToHost, stream[i]);
        }
