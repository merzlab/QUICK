		...
		const double* V2, const int* nbasis)
{
  ...
  double* devPtr_hold=0;
    ...
    const double h_zero = 0;
      ...
      // hold = o * x
            ...
            hipblasDgemm (cublasH, HIPBLAS_OP_N, HIPBLAS_OP_N, dim, dim, dim, &h_one, devPtr_o, dim,
		      devPtr_x, dim, &h_zero, devPtr_hold, dim);

    // o = x * hold
      hipblasDgemm (cublasH, HIPBLAS_OP_N,HIPBLAS_OP_N, dim, dim, dim, &h_one, devPtr_x, dim,
		      devPtr_hold, dim, &h_zero, devPtr_o, dim);


      //retrieve output matrix:	stat1=hipblasGetMatrix(dim, dim, sizeof(o[0]), devPtr_o, dim, o, dim);
      //
      //
      //      stat1=hipblasGetMatrix(dim, dim, sizeof(hold[0]), devPtr_hold, dim, hold, dim);
  ...
  cusolver_status = cusolverDnDsyevd_bufferSize(
	...
	//printf("started diag copy\n");    
        cudaStat1 = hipMemcpy((void*) vec, devPtr_o, sizeof(double)*dim*dim, hipMemcpyDeviceToHost);
        cudaStat2 = hipMemcpy((void*) E, devPtr_E, sizeof(double)*dim, hipMemcpyDeviceToHost);
        assert(hipSuccess == cudaStat1 && hipSuccess == cudaStat2);
	//printf("finished diag copy\n");	


    //stat1=hipblasGetMatrix(dim, dim, sizeof(vec[0]), devPtr_o, dim, vec, dim);


			  

    /*
    stat1=hipblasGetMatrix(dim, dim, sizeof(o[0]), devPtr_o, dim, o, dim);
    printf("o\n");
    ...
    stat1=hipblasGetMatrix(dim, dim, sizeof(o[0]), devPtr_E, dim, o, dim);
    printf("E\n");
    for(int i=0; i<dim*dim; i++)
      {
	printf("%d %f\n", i, o[i]);
    ...
    stat1=hipblasGetMatrix(dim, dim, sizeof(o[0]), devPtr_work, dim, o, dim);
    printf("work\n");
    for(int i=0; i<dim*dim; i++)
      {
	printf("%d %f\n", i, o[i]);
    ...
    */
    ...
    if (devPtr_o) hipFree(devPtr_o);
    if (devPtr_x) hipFree(devPtr_x);
    if (devPtr_hold) hipFree(devPtr_hold);
    if (devPtr_E) hipFree(devPtr_E);
    if (devPtr_devInfo) hipFree(devPtr_devInfo);
    if (devPtr_work) hipFree(devPtr_work);
