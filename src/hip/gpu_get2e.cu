...
//   nvtxRangePushA("Gradient 2e");
    QUICK_SAFE_CALL(hipLaunchKernelGGL(getGrad_kernel_sp, gpu->blocks, HIP_SP_2E_GRAD_THREADS_PER_BLOCK, 0, 0))

    QUICK_SAFE_CALL(hipLaunchKernelGGL(getGrad_kernel_spd, gpu->blocks, HIP_SPD_2E_GRAD_THREADS_PER_BLOCK, 0, 0))
    ...
    if (gpu->maxL >= 2) {
        //#ifdef HIP_SPDF
        // Part f-1
        QUICK_SAFE_CALL(hipLaunchKernelGGL(getGrad_kernel_spdf, gpu->blocks, HIP_SPDF_2E_GRAD_THREADS_PER_BLOCK, 0, 0))
        // Part f-2
        QUICK_SAFE_CALL(hipLaunchKernelGGL(getGrad_kernel_spdf2, gpu->blocks, HIP_SPDF2_2E_GRAD_THREADS_PER_BLOCK, 0, 0))
        // Part f-3
...
//   nvtxRangePushA("Gradient 2e");
   QUICK_SAFE_CALL(hipLaunchKernelGGL(getGrad_oshell_kernel_sp, gpu->blocks, HIP_SP_2E_GRAD_THREADS_PER_BLOCK, 0, 0))

   QUICK_SAFE_CALL(hipLaunchKernelGGL(getGrad_oshell_kernel_spd, gpu->blocks, HIP_SPD_2E_GRAD_THREADS_PER_BLOCK, 0, 0))
    ...
    if (gpu->maxL >= 2) {
        //#ifdef HIP_SPDF
        // Part f-1
        QUICK_SAFE_CALL(hipLaunchKernelGGL(getGrad_oshell_kernel_spdf, gpu->blocks, HIP_SPDF_2E_GRAD_THREADS_PER_BLOCK, 0, 0))
        // Part f-2
        QUICK_SAFE_CALL(hipLaunchKernelGGL(getGrad_oshell_kernel_spdf2, gpu->blocks, HIP_SPDF2_2E_GRAD_THREADS_PER_BLOCK, 0, 0))
        // Part f-3
        //    QUICK_SAFE_CALL(hipLaunchKernelGGL(getGrad_kernel_spdf3, gpu->blocks, gpu->gradThreadsPerBlock, 0, 0)))
        //#endif
    }
...
#ifdef USE_TEXTURE_CUTMATRIX
    hipBindTexture(NULL, tex_cutMatrix, gpu->gpu_sim.cutMatrix, gpu->nshell*gpu->nshell*sizeof(QUICKDouble));
#endif

#ifdef USE_TEXTURE_YCUTOFF
    hipBindTexture(NULL, tex_YCutoff, gpu->gpu_sim.YCutoff, gpu->nshell*gpu->nshell*sizeof(QUICKDouble));
#endif

#ifdef USE_TEXTURE_XCOEFF
    hipBindTexture(NULL, tex_Xcoeff, gpu->gpu_sim.Xcoeff, gpu->jbasis*gpu->jbasis*2*2*sizeof(QUICKDouble));
#endif
