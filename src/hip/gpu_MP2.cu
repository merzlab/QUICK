...
void upload_sim_to_constant_MP2(_gpu_type gpu){
    hipError_t status;
    status = hipMemcpyToSymbol(HIP_SYMBOL("devSim_MP2"), &gpu->gpu_sim, sizeof(gpu_simulation_type), 0, hipMemcpyHostToDevice);
...
void upload_para_to_const_MP2(){
    ...
    status = hipMemcpyToSymbol(HIP_SYMBOL(devTrans_MP2), trans, sizeof(int)*TRANSDIM*TRANSDIM*TRANSDIM);
