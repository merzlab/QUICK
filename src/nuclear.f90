...
! nuclearspdf.f90
subroutine nuclearattrafock1(Ips,Jps,IIsh,JJsh,NIJ1, &
      Ax,Ay,Az,Bx,By,Bz,Cx,Cy,Cz,Px,Py,Pz,iatom)
   use allmod
   ...
   double precision attra,aux(0:20)
   integer a(3),b(3), JID, ntri, I, J
   double precision Ax,Ay,Az,Bx,By,Bz,Cx,Cy,Cz,Px,Py,Pz,g, Agrad1, Agrad2, Agrad3, Bgrad1, Bgrad2, Bgrad3,Cgrad1, Cgrad2, Cgrad3
   double precision AA(3),BB(3),CC(3),PP(3)
   common /xiaoattra/attra,aux,AA,BB,CC,PP,g
      ...
      do Jang=quick_basis%Qstart(JJsh),quick_basis%Qfinal(JJsh)
         ...
         do III=III1,III2
            ...
            do JJJ=max(III,JJJ1),JJJ2
               ...
               Cgrad1=Xconstant2*attraxiaoopt(1,itemp1,itemp2,0)
               Cgrad2=Xconstant2*attraxiaoopt(2,itemp1,itemp2,0)
               Cgrad3=Xconstant2*attraxiaoopt(3,itemp1,itemp2,0)

               itemp1new=trans(quick_basis%KLMN(1,III)+1,quick_basis%KLMN(2,III),quick_basis%KLMN(3,III))
               Agrad1=2.0d0*Xconstant2* &
                     quick_basis%gcexpo(ips,quick_basis%ksumtype(IIsh))*attraxiao(itemp1new,itemp2,0)
               if(quick_basis%KLMN(1,III).ge.1)then
                  itemp1new=trans(quick_basis%KLMN(1,III)-1,quick_basis%KLMN(2,III),quick_basis%KLMN(3,III))
                  Agrad1=Agrad1-Xconstant2* &
               ...
               quick_qm_struct%fd(iAstart+1,JID)= quick_qm_struct%fd(iAstart+1,JID)+ Agrad1
               quick_qm_struct%fd(iBstart+1,JID)= quick_qm_struct%fd(iBstart+1,JID)+ Bgrad1
               quick_qm_struct%fd(iCstart+1,JID)= quick_qm_struct%fd(iCstart+1,JID)+ Cgrad1

               quick_qm_struct%fd(iAstart+2,JID)= quick_qm_struct%fd(iAstart+2,JID)+ Agrad2
               quick_qm_struct%fd(iBstart+2,JID)= quick_qm_struct%fd(iBstart+2,JID)+ Bgrad2
               quick_qm_struct%fd(iCstart+2,JID)= quick_qm_struct%fd(iCstart+2,JID)+ Cgrad2

               quick_qm_struct%fd(iAstart+3,JID)= quick_qm_struct%fd(iAstart+3,JID)+ Agrad3
...
End subroutine nuclearattrafock1
