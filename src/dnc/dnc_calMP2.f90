...
subroutine calmp2divcon
  ...
  double precision Xiaotest,testtmp,emp2total
  ...
  do itt=1,np
    ...
    ! determine the occupied and virtual orbitals
     ...
     write(ioutfile,*) 'Frag. #   | Frag EMP2     | EMP2 diff with previous frag'
     write(ioutfile,*) itt,quick_qm_struct%EMP2,quick_qm_struct%EMP2-emp2temp

     emp2temp=quick_qm_struct%EMP2
     emp2total=quick_qm_struct%EMP2+quick_qm_struct%EMP2      
...
!     deallocate(mp2shell)
  ...
  write(ioutfile,*) ' ' 
  write(ioutfile,*) 'Total EMP2=', emp2total
  write(ioutfile,*) ' ' 
